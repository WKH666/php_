var myScroll=false,yy={
	...
	xiang:function(oi){
		...
		var ids = d.id,nus=d.modenum,modne=d.modename,num=d.num;
		if(!ids)return;
		if(!num)return;
		//if(!nus||nus=='undefined')nus = this.num;
		var url='task.php?a=x&num='+num+'&mid='+ids+'&show=we';
		js.location(url);
	},
	suboptmenu:{},
	...
	showdata:function(a){
		...
		switch(a.num){
			case 'flow':
					for(i=0;i<len;i++){
						...
						if(d.showtype=='line' && d.title){
							...
							if(d.cont)s+='<div onclick="yy.xiang('+oi+')" class="cont">'+d.cont.replace(/\n/g,'<br>')+'</div>';
							if(d.id && d.modenum){
								//这里吧操作隐藏掉
								//s+='<div class="xq r-border-t"><font onclick="yy.showmenu('+oi+')">操作<i class="icon-angle-down"></i></font><span onclick="yy.xiang('+oi+')">详情&gt;&gt;</span>';
			//					s+='<div class="xq r-border-t" style="height:18px;"><span onclick="yy.xiang('+oi+')">详情&gt;&gt;</span>';
			//					s+='</div>';
							}
							if(d.statustext)s+='<div style="color:'+d.statuscolor+';opacity:0.7;border:1px solid '+d.statuscolor+';" class="zt">'+d.statustext+'</div>';
							s+='</div>';
						}
					...
					for(i=0;i<len;i++){
						...
						if(d.showtype=='line' && d.title){
							...
							if(d.project_name ){
									...
									s+='<div onclick="yy.xiang('+oi+')" class="tit">项目名称：'+d.project_name+'</div>';
							}
							if(d.optdt)s+='<div class="dt">'+d.optdt+'</div>'
							if(d.project_head && d.project_number && d.project_apply_time){
								s+='<div onclick="yy.xiang('+oi+')" class="cont">项目负责人：'+d.project_head+'</br>项目编号：'+d.project_number+'</br>申报时间：'+d.project_apply_time+'</div>';
							}
			...
			//					s+='</div>';
							...
							if(d.statustext)s+='<div style="color:blue;opacity:0.7;border:1px solid '+d.statuscolor+';" class="zt">'+d.statustext+'</div>';
							s+='</div>';
						}
						this.showobj.append(s);
					}
					...
					if(count>0){
						this.nowpage = a.page;
						s = '<div class="showblank" id="showblank">';
						//共'+count+'条记录';
