...
function optmenuclass(o1,num,id,obj,mname,oi, cola){
	...
	this.getupgurl=function(str){
		/* 普及月项目申报查看活动资料
		*   /1.获取流程模块id，
			//2.根据流程模块id查出flow_bill表的当前模块单据，取的字段nowcourse_id，mid
			//3.流程模块主表加nowcourseid字段，根据获取的mid更新当前流程模快主表的nowcourse_id字段
			//4.流程模块主表加nowcheckid字段(当前环节审核人id) 获取flow表里的nowcheckid
		 */
		...
		if(lx=='zidinyi')url='?a=common&m=detail&d=flow&num='+mk+'';
		if(lx=='chakan')url='?a=check&m=input&d=flow&num='+mk+'&mid='+this.id+'';
		if(cs)url+='&'+cs;
		console.log(url);
		return url;
	};
