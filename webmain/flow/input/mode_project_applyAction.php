<?php
/**
 *	此文件是流程模块【project_apply.项目申报】对应接口文件。
 *	可在页面上创建更多方法如：public funciton testactAjax()，用js.getajaxurl('testact','mode_project_apply|input','flow')调用到对应方法
 */
class mode_project_applyClassAction extends inputAction {
	...
	protected function saveafter($table, $arr, $id, $addbo) {
		if ($addbo) {
			...
			//更新填入项目编号
			$project_apply = m('project_apply') -> getone('id=' . $id);
			$project_select_num = m('option') -> getval_na($project_apply['project_select'], '', 0);
			//分类编号
			//项目编号 = 项目分类 + 申报年份 + 三位数的序号
			if ($project_apply['project_number'] == '') {
				$rows = m('project_apply') -> rows('1=1');
				$num = sprintf("%03d", $rows);
				$data['project_number'] = $project_select_num . date('Y', strtotime($project_apply['applydt'])) . $num;
			}
            m('project_apply') -> update($data, 'id=' . $id);
		}
	}
