...
function initbody() {
	...
	for(i = 0; i < len; i++) {
		...
		if(arr[i].islu == '1' && arr[i].iszb == '0') {
			...
			if(flx == 'htmlediter') dfk.htmlediter(arr[i].fields);
		}
	}
	dfk.initsubtable();
	if(nfid == '') {
...
var dfk = {
	...
	savesss: function() {
		...
		for(i = 0; i < len; i++) {
			...
			if(val && flx == 'textarea') {
				if(checkInit.init('textArea', val.length) == false) {
					layer.closeAll();
	...
	showdata: function() {
		...
		if(smid == '0' || smid == '') {
			...
			js.ajax(geturlact('getdata'), { mid: smid, flownum: moders.num }, function(str) {
				dfk.showdataback(js.decode(str));
			}, 'post', function() {
				js.setmsg('error:内部错误,可F12调试');
			});
		}
	...
	initinput: function() {
		...
		for(i = 0; i < o.length; i++) {
				...
				$.rockupload({
					...
					'onsuccess': function(f, gstr) {
						...
						if(tsye == 'img') {
							...
							$('#fileview_' + sna + '').html(dfk.showfilestr(d));
							form(sna).value = d.id;
						}
					}
				});
			...
			if(tsye == 'file' && val && val > 0) {
				far = farr['f' + val];
				if(far) {
					$('#fileview_' + sna + '').html(dfk.showfilestr(far));
				} else {
					form(sna).value = '0';
				}
			}
			...
			for(j = 0; j <= 3; j++) {
				...
				if(subds)
					for(i = 0; i < subds.length; i++) {
						...
						if(form('xuhao' + j + '_' + i + '')) {
							dfk.adddatarow(j, i, subds[i]);
						} else {
							dfk.insertrow(j, subds[i], true);
						}
					}
			}
			dfk.initinput();
