...
<script>
    $(document).ready(function(){
        ...
        var c = {
            ...
            initshow: function (data) {
                 if ($('#op1').val()==data.send_type) { $("#op1").attr('selected',true);}
                 if ($('#op2').val()==data.send_type) { $("#op2").attr('selected',true);}
                $("#receive_id").val(data.receive_id);
                $("#send_titles").val(data.send_title);
                $("#send_remark").val(data.send_remark);
                // $("#position").val(data.position);
                if($("#is_send0").val()==data.is_send){$("#is_send0").attr('checked',true);}
                if($("#is_send1").val()==data.is_send){$("#is_send1").attr('checked',true);}
    ...
    function draft() {
        ...
        if (id1!=null) {
            js.ajax(js.getajaxurl('get_save', 'mail', 'main'), {id:id,results_id: id1.id,send_type:send_type,receive_id:receive_id,remark:remark,titles:titles,is_send:is_send,send_status:0}, function (data) {
                if (data){
                    closenowtabs();
                    try {
            ...
            js.ajax(js.getajaxurl('got_save', 'mail', 'main'), {id:id,send_type:send_type,receive_id:receive_id,remark:remark,titles:titles,is_send:is_send,send_status:0}, function (data) {
                if (data==true){
                    closenowtabs();
                    try {
    ...
    function sub() {
        ...
        var titles=$('#send_titles').val();
        var is_send=$(':radio[name="is_send"]:checked').val();
        //{results_id: id1.id,del_id:annualreport_id,upload_status:1}
        if (id1!=null) {
            js.ajax(js.getajaxurl('get_data', 'mail', 'main'), {id:id,results_id: id1.id,send_type:send_type,receive_id:receive_id,remark:remark,titles:titles,is_send:is_send,send_status:1}, function (data) {
                if (data){
                    closenowtabs();
                    try {
                        assessmentList.reload();
            ...
            js.ajax(js.getajaxurl('got_data', 'mail', 'main'), {id:id,send_type:send_type,receive_id:receive_id,remark:remark,titles:titles,is_send:is_send,send_status:1}, function (data) {
                if (data==true){
                    closenowtabs();
                    try {
...
<div>
    ...
    <form class="three_columns">
        <div class="form-group">
            <label>群发类型:</label>
            <select type="text" class="form-control"  id="send_type">
    ...
    <form class="three_columns">
        <div class="form-group">
            <label>接收者:</label>
            <input type="text" class="form-control"  id="receive_id" placeholder="请选择" >
    ...
    <form class="three_columns">
        <div class="form-group">
            <label>发送标题:</label>
            <textarea  class="form-control"  id="send_titles" placeholder="请输入"></textarea>
        </div>
    </form>
    <form class="three_columns">
        <div class="form-group">
            <label>发送说明:</label>
            <textarea class="form-control"  id="send_remark" placeholder="请输入"></textarea>
        </div>
    </form>
    <form class="three_columns">
        <div class="form-group">
                            ...
                            <div class="searchAc1">
                                <ul>
                                    <li style="margin: 5px">
                                        ...
                                        <button type="button" click="reset" id="reset2" class="btn btn-default " style="margin-left: 15px">重置</button>
                                    </li>
                                </ul>
                            </div>
                        </div>
                            ...
                            <div class="searchAc1">
                                <ul>
                                    <li style="margin: 5px">
                                        ...
                                        <button type="button" onclick="czjs()" id="reset" class="btn btn-default " style="margin-left: 15px">重置</button>
                                    </li>
                                </ul>
                            </div>
                        </div>
