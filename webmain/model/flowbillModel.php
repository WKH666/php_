	...
	*/
	public function getrecord($uid, $lx, $page, $limit, $glx=0)
	{
		...
		if($lx=='daiban_daib' || $lx=='daiban_def'){
			$where	= '`status` not in(1,2,5) and '.$this->rock->dbinstr('nowcheckid', $uid);
			$isdb	= 1;
		}
	...
	{
		$where	= '`status` not in(1,2,5) and isdel=0 and '.$this->rock->dbinstr('nowcheckid', $uid);
		$to 	= $this->rows($where);
		return $to;
	}
...
//	}
	 ...
	 public function homelistshow(){
		...
		foreach($rows as $k=>$rs){
		    //2020-12-11修改
			//$project_apply = $this->db->getone("whole_projects",'id='.$rs['id'].' and num="'.$rs['modenum'].'"');
            $mode_num = $rs['modenum'];
            $project_apply = m("$mode_num")->getone('id='.$rs['id'].'');
            $cont=null;
            if ($mode_num=='project_coursetask'){
                $cont = '【'.$rs['modename'].'】名称：'.$project_apply['course_name'].'，';
            }else{
                $cont = '【'.$rs['modename'].'】名称：'.$project_apply['project_name'].'，';
            }
			if(!contain($rs['statustext'],'<font')){
				$cont.= '<font color="'.$rs['statuscolor'].'">'.$rs['statustext'].'</font>';
			}else{
