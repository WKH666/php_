...
class adminClassModel extends Model
{
	...
	*/
	...
	{
		$rows = $this->getall("`status`=1",'id,name,deptid,deptname,deptallname,mobile,ranking,tel,face,sex,email,pingyin,groupname','sort,name');
		$py   = c('pingyin');
		foreach($rows as $k=>$rs){
		   $group_ids = explode(',',$rs['groupname']);
		    $group_text = '';
		    foreach ($group_ids as $v){
		        $rm = m('group')->getone("id = $v");
		        if ($rm){
                    $group_text.=$rm['name'].',';
                }
            }
            $rows[$k]['groupname'] = $group_text;
	...
	*/
     ...
     * @param $value
     */
	public function getKeyValueList($where = '1=1', $key = 'id', $value = 'name')
    {
        $admin = $this->getall($where, "`$key`,`$value`");
        if ($admin) $admin = array_column($admin, $value, $key);
        return $admin;
    }
}
