...
class fileClassModel extends Model
{
	...
	public function getfilewhere($mtype, $mid, $where='')
	{
		$rows	= $this->getall("`mtype`='$mtype' and `mid` in($mid) $where order by `id`",'id,`mid`,filename,filepath,filesizecn,filesize,fileext,optname,adddt');
		return $rows;
	}
