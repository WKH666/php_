...
class flowModel extends Model
{
	...
	//下一步审核人中默认审核人 $mid当前项目在flow_bill中的id  $buzou_id 流程id
	...
	*/
	public function getdatalog($lx=0)
	{
		...
		$arr['isgbcllc']    = (int)$this->rock->arrvalue($this->moders,'isgbcllc','0'); //是否不显示处理界面的处理流程  0/1 否/是
...
//$arr['flowinfordd']='dd';
	...
	private function getsubdata($xu, $rows)
	{
		...
		if($this->db->count > 0){
			...
			$cont 	 	= c('html')->createrows($rows, $headstr, '#000000', 'noborder');
		}
		return $cont;
	}
	...
	*/
	public function getflowinfor()
	{
		...
		//上传
		if(isset($nowcur['isuploadfile'])){
			$sarr['isuploadfile'] = $nowcur['isuploadfile'];//判断是否可上传文件
		}else{
			$sarr['isuploadfile']="1";
		}
		...
		//下一步
		$buzou=m('flow_bill')->getone('mid='.$this->id,'nowcourseid');
		$next=$this->nextflowcheckname($this->id,$buzou['nowcourseid']);

		if($next){
			$sarr['next_moren_nanme'] 	= $next['checkname'];
			$sarr['next_moren_id'] 	= $next['checkid'];
		}
		...
		//  ',通过|green,不通过|red'   这是默认值不能修改,再添加如下:  ',通过|green,不通过|red,拒绝|gray|5'

		//系统默认的通过不通过，改为通过（通过）、修改后通过（不通过）、不通过（拒绝）
		$actstr	= ',通过|black,退回修改|black,不通过|black|5';
		...
		$sarr['ischecksuccess']    = (int)$this->rock->arrvalue($this->moders,'ischecksuccess','0'); //处理结果是否默认选择 0/1 否/是
	...
	*/
	public function getstatus($rs, $statusstr='',$other='', $glx=0)
	{
		...
		$statusara 	= array();
		$colorsa   	= array('black','green','red','#ff6600','#526D08','#888888','','','','','','','','','','','','','');
		if(isempt($statusstr))$statusstr =  '待?处理|,已审核|,未通过|';
	...
	*/
	...
	public function getlog()
	{
		...
		foreach($rows as $k=>$rs){
			...
			$rows[$k]['optdt'] 	= $dts->stringdt($rs['optdt'], 'Y-m-d(周w) H:i:s');
		}
		//读取相关文件
		if($idss!=''){
			...
			if($farr)foreach($rows as $k=>$rs){
				...
				$rows[$k]['yijian']= $this->strappend($rs['explain'], "", '<br>');
				$rows[$k]['explain_file']=$fstr;
	...
	public function submit($na='', $sm='')
	{
		...
		if($this->isflow == 1){
			...
			//$marr['status'] = 0;
			$this->rs['status'] = 0;
			$this->update($marr, $this->id);//end
			$farr = $this->getflow();

			//第一步自定义审核人
			if($farr['nowcourseid']>0){
				$sysnextoptid 	= $this->rock->post('sysnextoptid');
				$sysnextopt 	= $this->rock->post('sysnextopt');
				$sysnextcustidid = (int)$this->rock->post('sysnextcustidid');
	...
	*/
	...
	public function nexttodo($nuid, $type, $sm='', $act='')
	{
		...
		if($type=='submit' || $type=='next'){
			$cont = '你有['.$this->adminname.']的['.$this->modename.',单号:'.$this->modename.']需要处理';
			if($sm!='')$cont.='，说明:'.$sm.'';
		}
		//审核不通过
		if($type == 'nothrough'){
	...
	*/
	...
	{
		$rows 	= $this->db->getrows('[Q]flow_menu',"`setid`='$this->modeid' and `status`=1",'id,wherestr,name,statuscolor,statusvalue,num,islog,issm,type,upgcont','`sort`');
		$arr 	= array();
		if($rows){

			foreach($rows as $k=>$rs){

                $rs['upgcont'] = base64_decode($rs['upgcont']);
...
//			}
			...
			if($chearr['ischeck']==1 && $chearr['nowstatus']!=5 ){
		...
		$tf_bianji=m('flow_bill')->getone('modeid='.$this->modeid.' and uid='.$this->adminid);
		//已作废则不能编辑
		if($this->iseditqx()==1 && $this->getflowinfor()['nowstatus']!=5 && !empty($tf_bianji)){
