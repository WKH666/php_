...
/**
*	来自：广州迈峰网络有限公司
*	网址：http://xh829http://www.minephone.com/.com/
*	系统的核心文件之一，处理工作流程模块的。
*/
class flowModel extends Model
{
	...
	*/
	public function getdatalog($lx=0)
	{
		...
		$fstr			 = $fobj->getstr($this->mtable, $this->id, 1);
		$issubtabs		 = 0;
		if($fstr != ''){
		...
		foreach($this->fieldsarr as $k=>$rs){
			if($rs['fieldstype']=='uploadfile'){
				...
				if($fval>0){
					...
					$data[$fid] = $fobj->getfilestr($ftrs, 1);
				}
			}
		}
	...
	*/
	...
	{
		$rows  	= $this->getflowpipei($this->uid);
		$this->flowarr 	= array();
	...
	*/
	...
	private function addcheckname($courseid, $uid, $uname, $onbo=false)
	{
		$uida 	= explode(',', ''.$uid.'');
		$uidan 	= explode(',', $uname);
