...
$(document).ready(function(){
	...
	var id = params.id;
	if(!id)id = 0;
	var submitfields = 'mtype,mid,file_ids,accept_user_name,accept_msg,accept_time';
	var h = $.bootsform({
		window:false,rand:'{rand}',tablename:'mf_accept',
		url:js.getajaxurl('saveaccept','extrainfo','main'),
		modenum:'mf_accept',
		submitfields:submitfields,
		success:function(){
			closenowtabs();
			try{acceptlist.reload();}catch(e){}
		}
	});
	h.forminit();
	if(id!=0){//如果是编辑，则获取相应的表单数据
		h.load(js.getajaxurl('loadform','extrainfo','main',{id:id,table:jm.base64encode('accept')}));
		js.ajax(js.getajaxurl('getupfiles','extrainfo','main'),{id:id,table:jm.base64encode('accept')},function(da){
...
<div  style="padding:10px;width:720px">
	...
	<form name="form_{rand}">
		<input id="id_{rand}" name="accept_id" value="0" type="hidden" />
		...
		<div class="headerTitle" style="width: 100%;">
			<div>
				<span class="titleContent f20" >录入验收信息</span>
			</div>
		</div>
		...
		<tr>
			...
			<td class="tdinput" width="34%"><input name="accept_user_name" placeholder="" maxlength="10" class="form-control"></td>
		</tr>
		<tr>
			<td align="left" width="4%">验收说明：</td>
			<td class="tdinput" width="34%"><textarea class="form-control" name="accept_msg" r ows="3"></textarea></td>
		</tr>
		<tr>
			<td colspan="2">
		...
		<tr>
			<td align="left" width="4%">验收时间：</td>
			<td align="left" style="width: 33%;">
			<div style="width:200px"  class="input-group">
				<input readonly class="form-control" name="accept_time" id="dt1_{rand}" >
