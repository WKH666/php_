...
$(document).ready(function(){
	...
	a = $('#view_{rand}').bootstable({
		url:js.getajaxurl('payment','extrainfo','main',{mid:lx}),fanye:true,storeafteraction:'extrainfoafter',storebeforeaction:'dataauthbefore',
		columns:[{
			text:'项目名称',dataIndex:'project_name'
		},{
			text:'付款负责人',dataIndex:'payment_user_name'
		},{
			text:'付款时间',dataIndex:'payment_time',sortable:true,renderer:function(v,d){
				if(d.is_payment==0)return '暂未付款';
				else return v;
			}
		},{
			text:'库性质',dataIndex:'project_xingzhi'
		},{
			text:'付款说明',dataIndex:'payment_msg'
		},{
...
//录入付款信息
function payment_add(lx,mtype,project_name){
	paymentlist = a;
	addtabs({num:'payment_add'+lx,url:'main,extrainfo,fkedit,mid='+lx+',mtype='+mtype+',project_name='+project_name,icons:'icon-bookmark-empty',name:'录入['+project_name+']验收信息'});
}

//查看付款信息
function payment_check(mid,id,project_name){
	var tab_id = 'payment-'+id;//这个请求不能直接用1,2,3这样的方式传值，要表名_id
	addtabs({num:'payment_file_'+id,url:'main,archives,files,tab_id='+tab_id+',mid='+mid,icons:'icon-bookmark-empty',name:'['+project_name+']验收信息文件'});
}

//编辑付款信息
function payment_edit(lx,mtype,id,project_name){
	paymentlist = a;
	addtabs({num:'payment_edit'+lx,url:'main,extrainfo,fkedit,id='+id+',mtype='+mtype+',mid='+lx+',project_name='+project_name,icons:'icon-bookmark-empty',name:'编辑['+project_name+']验收信息'});
}
...
<section class="serachPanel selBackGround">
	<div class="searchAPanel">
		...
		<div class="searchAc1">
			<ul>
				<li><span class="reviewContent stateContent">付款金额</span></li>
				<li><input class="form-control txtPanel"  id="project_head_{rand}" placeholder="请输入付款金额"></li>
			</ul>
		</div>
		<div class="searchAc1" style="/*width: 33%;*/min-width: 440px;">
			<ul>
				<li><span class="reviewContent stateContent">付款日期</span></li>
				<li>
...
<div class="selBackGround">
	...
	<tr>
		...
		<td align="center" style="width: 33%;">付款金额<input class="form-control" style="width:200px" id="project_head_{rand}" placeholder="请输入项目负责人名称"></td>
		<td align="center" style="width: 33%;">
			<div style="width:200px"  class="input-group">
				<input placeholder="付款日期" readonly class="form-control" id="dt1_{rand}" >
				<span class="input-group-btn">
