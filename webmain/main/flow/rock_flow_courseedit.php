...
$(document).ready(function(){
	...
	var h = $.bootsform({
		...
		submitfields:'setid,name,num,checktype,checktypeid,checktypename,checkfields,sort,whereid,explain,status,courseact,checkshu,recename,receid,pid,iszf,isuploadfile',
		requiredfields:'name',
		success:function(){
			closenowtabs();
		...
		submitcheck:function(d){
			...
			if(d.checktype=='role'&&d.checktypename=='')return '请选择角色';
			return '';
		}
	});
	h.forminit();
	...
	var c = {
		...
		changetype:function(lx){
            ...
            if(v=='role'){
                $('#checktext_{rand}').html('请根据角色选择审核人：');
                $('#checkname_{rand}').show();
            }
			if(v=='user'){
		...
		setstatus:function(){
			...
			if(statusstr=='' || statusstr == null)statusstr='待处理,已完成,不通过';
...
<div  style="padding:10px;width:700px">
	...
	<form name="form_{rand}">
		...
		<tr>
			...
			<td class="tdinput">
                <select class="form-control" name="checktype">
                    ...
                    <option value="auto">自定义(写代码上)</option>
                    <option value="apply">申请人</option>
                    <option value="opt">操作人</option>
                    <option value="change">由上步指定</option></select>
            </td>
			<td align="right" id="checktext_{rand}" nowrap></td>
			<td class="tdinput" id="checkname_{rand}" style="display:none">
				<div class="input-group" style="width:100%">
					<input class="form-control"  name="checktypename" readonly>
		...
		<tr>
			...
			<td class="tdinput" colspan="3"><input name="courseact" class="form-control"><div style="padding-top:0px" class="tishi"><a href="javascript:;" click="setstatus">[设置]</a>默认是：通过,退回修改,不通过。多个,分开</div></td>
		</tr>
        <!--审核处理表单-->
		<tr>
		...
		<tr>
			...
			<td class="tdinput" colspan="3">
				...
				<label><input name="isuploadfile" checked value="1" type="checkbox">是否可上传文件</label>
			</td>
		</tr>
