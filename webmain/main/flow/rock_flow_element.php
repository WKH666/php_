...
$(document).ready(function(){
	...
	var c = {
		...
		clickwin:function(o1,lx){
			js.confirm('确定要编辑表单元素吗？数据改变后可能影响系统功能,将不可恢复！',function(jg){
				if(jg=='yes'){
					if(modeid==0)return;
...
<div>
	...
	<td width="80%" align="left" nowrap style="padding-left:10px;">
		<div class="btn-group">
		<button class="btn btn-default" id="luc_{rand}" disabled click="inputs,0" type="button">PC录入页布局</button>
		<button class="btn btn-default" id="luz_{rand}" disabled click="zhanshi,0" type="button">PC展示</button>
		<button style="display: none;" class="btn btn-default" id="luzz_{rand}" disabled click="zhanshi,1" type="button">手机展示</button>
		<button class="btn btn-default" id="lu_{rand}" disabled click="lulu,0" type="button"><i class="icon-desktop"></i> PC录入页</button>
		<button style="display: none;" id="lum_{rand}" disabled click="lulu,1" type="button"><i class="icon-tablet"></i> 手机录入页</button>
		</div>
	</td>
