...
$(document).ready(function(){
	...
	a = $('#view_{rand}').bootstable({
		url:js.getajaxurl('purchase','construction','main',{mid:lx}),data:{is_ys:true},fanye:true,storeafteraction:'constructionafter',storebeforeaction:'dataauthbefore',
		columns:[{
            text:'项目编号',dataIndex:'budget_code'
        }, {
            text:'项目名称',dataIndex:'budget_name'
        }, {
			text:'验收负责人',dataIndex:'it_order'
		}, {
			text:'验收时间',dataIndex:'accept_date'
		}, {
			text:'存在问题',dataIndex:'quality_cdt'
		}, {
...
//		}
	...
	var c = {
		...
		search:function(){
			...
			a.setparams({
				is_ys:true,
				accept_date: time_frame,
				budget_name:get('project_name_{rand}').value,
				it_order:get('project_head_{rand}').value,
			},true);
		},
...
<section class="serachPanel selBackGround">
	<div class="searchAPanel">
		...
		<div class="searchAc1">
			<ul>
				<li><span class="reviewContent stateContent">验收人员</span></li>
