...
$(document).ready(function(){
	...
	a = $('#view_{rand}').bootstable({
		url:js.getajaxurl('evaluation','construction','main',{mid:lx}),fanye:true,storeafteraction:'constructionafter',storebeforeaction:'dataauthbefore',
		columns:[{
		...
		{
			text:'考评时间',dataIndex:'evaluation_time',sortable:true,renderer:function(v,d){
				if(d.is_evaluation==0)return '暂未考评';
				else return v;
			}
		},{
...
//录入考评信息
function evaluation_add(lx,mtype,project_name){
	evaluationlist = a;
	addtabs({num:'evaluation_add'+lx,url:'main,construction,jxkpedit,mid='+lx+',mtype='+mtype+',project_name='+project_name,icons:'icon-bookmark-empty',name:'录入['+project_name+']验收信息'});
}

//查看考评信息
function evaluation_check(mid,id,project_name){
	var tab_id = 'evaluation-'+id;//这个请求不能直接用1,2,3这样的方式传值，要表名_id
	addtabs({num:'evaluation_file_'+lx,url:'main,archives,files,tab_id='+tab_id+',mid='+mid,icons:'icon-bookmark-empty',name:'['+project_name+']验收信息文件'});
}

//编辑考评信息
function evaluation_edit(lx,mtype,id,project_name){
	evaluationlist = a;
	addtabs({num:'evaluation_edit'+lx,url:'main,construction,jxkpedit,id='+id+',mtype='+mtype+',mid='+lx+',project_name='+project_name,icons:'icon-bookmark-empty',name:'编辑['+project_name+']验收信息'});
}
...
<div class="selBackGround">
	...
	<tr>
		...
		<td align="center" style="width: 33%;">
			<div style="width:200px"  class="input-group">
				<input placeholder="考评时间" readonly class="form-control" id="dt1_{rand}" >
	...
	<tr>
		...
		<td align="center" style="width: 33%;">项目编号<input class="form-control" style="width:200px" id="project_number_{rand}" placeholder="请输入项目编辑"></td>
...
<section class="serachPanel selBackGround">
	<div class="searchAPanel">
		...
		<div class="searchAc1" style="/*width: 33%;*/min-width: 440px;">
			<ul>
				<li><span class="reviewContent stateContent">考评日期</span></li>
				<li>
