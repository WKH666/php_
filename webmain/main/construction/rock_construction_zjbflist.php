...
$(document).ready(function(){
	...
	a = $('#view_{rand}').bootstable({
		url:js.getajaxurl('appropriation','construction','main',{mid:lx}),fanye:true,storeafteraction:'constructionafter',storebeforeaction:'dataauthbefore',
		columns:[{
...
function appropriation_add(lx,mtype,project_name){
	appropriationlist = a;
	addtabs({num:'appropriation_add'+lx, url:'main,construction,zjbfedit,mid='+lx+',mtype='+mtype+',project_name='+project_name, icons:'icon-bookmark-empty',name:'录入['+project_name+']经费信息'});
}

//查看拨付信息
function appropriation_check(mid,id,project_name){
	var tab_id = 'appropriation-'+id;//这个请求不能直接用1,2,3这样的方式传值，要表名_id
	//addtabs({num:'appropriation_file_'+lx,url:'main,archives,files,tab_id='+tab_id+',mid='+mid,icons:'icon-bookmark-empty',name:'['+project_name+']经费信息文件'});
    addtabs({num:'appropriation_detail_'+lx, url:'main,construction,zjbflook,tab_id='+tab_id+',mid='+mid+',id='+id, icons:'icon-bookmark-empty',name:'['+project_name+']经费信息文件'});
}

//编辑拨付信息
function appropriation_edit(lx,mtype,id,project_name){
	appropriationlist = a;
	addtabs({num:'appropriation_edit'+lx, url:'main,construction,zjbfedit,id='+id+',mtype='+mtype+',mid='+lx+',project_name='+project_name, icons:'icon-bookmark-empty',name:'编辑['+project_name+']经费信息'});
}
