...
<script>
    ...
    $(document).ready(function(){
        ...
        a = $('#projectend_{rand}').bootstable({
            tablename:'m_pxm_relation',celleditor:true,
            url:js.getajaxurl('endpciglist','project_comment','main', {pici_id:params.pici_id,mtype:'project_end'}),
            fanye:true,modename:'网评列表',
            celleditor:true,storeafteraction:'endpciglistafter',
    ...
    //评分
    function endpcig_comment(pici_id,num,mid,uid){
        if(get('tabs_add_norm')) closetabs('add_norm');
        if(get('tabs_edit_norm')) closetabs('edit_norm');
        addtabs({num:'edit_norm',url:'main,project_comment,expert,pici_id='+pici_id+',num='+num+',mid='+mid+',uid='+uid+',type=project_end',icons:'icon-bookmark-empty',name:'项目评分'});
        thechangetabs('edit_norm');
    }

    //查看
    function endpcig_look(pici_id,num,mid){
        expert_list=a;
        if(get('tabs_look_norm')) closetabs('look_norm');
        addtabs({num:'look_norm',url:'main,project_comment,expert_look,pici_id='+pici_id+',num='+num+',mid='+mid+',type=project_end',icons:'icon-bookmark-empty',name:'查看评分'});
    ...
    function comment_capgao(pici_id,num,mid,com_status){
        ...
        if(get('tabs_add_norm')) closetabs('add_norm');
        if(get('tabs_edit_norm')) closetabs('edit_norm');
        addtabs({num:'edit_norm',url:'main,project_comment,expert_edit,pici_id='+pici_id+',num='+num+',mid='+mid+',type=project_end',icons:'icon-bookmark-empty',name:'项目评分'});
        thechangetabs('edit_norm');
    }
    ...
    function save_score() {
        $.ajax({
            ...
            type:'post',
            dataType:'json',
            data:{project_id:selected_id,pici_id:pici_id,types:'project_end'},
            success:function (res) {
