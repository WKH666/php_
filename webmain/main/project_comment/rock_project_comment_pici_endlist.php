...
//查看
function look(id){
	if(get('tabs_look_batch')) closetabs('pcig_endinfo');
	addtabs({num:'pcig_endinfo',url:'main,project_comment,pcig_endinfo,pici_id='+id+',pinshentype='+pinshentype,icons:'icon-bookmark-empty',name:'查看批次'});
	thechangetabs('pcig_endinfo');
}
...
function project_enddow(pici_id,lx){
...
function export_expertinfo() {
    var url=getRootPath()+'/?d=main&m=project_comment&a=getExcel&lx=4&pinshenType=project_end';
    js.open(url,800,500);
}
