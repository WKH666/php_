var MODE	= '',ACTION = '',DIR='',PROJECT='',HOST='',PARAMS='',QOM='pl_',apiurl='',token='',device='',ISDEMO=false;
...
js.tanbody=function(act,title,w,h,can1){
	...
	s+='	<div class="title title1 '+can.titlecls+'" style="-moz-user-select:none;-webkit-user-select:none;user-select:none;">';
	s+='		<table border="0"  width="100%" cellspacing="0" cellpadding="0"><tr>';
	s+='			<td height="34" style="font-size:13px; font-weight:bold;color:white; padding-left:8px" width="100%" onmousedown="js.move(\''+mid+'\')" id="'+act+'_title">'+title+'</td>';
	s+='<td><div onmouseover="this.style.backgroundColor=\'#C64343\'" onmouseout="this.style.backgroundColor=\'\'" style="padding:0px 8px;height:34px;overflow:hidden;cursor:pointer;" onclick="js.tanclose(\''+act+'\',\''+can.guanact+'\')"><div id="'+act+'_spancancel" style="height:16px;overflow:hidden;width:16px;background:url(images/wclose.png);margin-top:10px"></div></div></td>';
	s+='		</tr></table>';
	s+='	</div>';
...
js.xpbody=function(act,type){
	...
	var bs='<div id="xpbg_bodydds" xpbody="'+act+'" oncontextmenu="return false" style="position:absolute;display:none;width:'+W+'px;height:'+H+'px;filter:Alpha(opacity=30);opacity:0.5;left:0px;top:0px;background-color:#000000;z-index:80"></div>';
	$('body').prepend(bs);
...
js.msg = function(lx, txt,sj){
	...
	if(lx=='msg')txt='<font color="#fff">'+txt+'</font>';var t=50;
	var s = '<div onclick="$(this).remove()" id="msgshowdivla" style="position:fixed;top:'+t+'%;z-index:200;" align="center"><div style="padding:8px 20px;background:rgba(65,154,241,0.9);color:white;font-size:16px;">'+txt+'</div></div>';
	$('body').append(s);
	var w=$('#msgshowdivla').width(),l=(winWb()-w)*0.5;
	$('#msgshowdivla').css('left',''+l+'px');
	if(sj>0)this.msgshowtime= setTimeout("$('#msgshowdivla').remove()",sj*600);
}
...
js.ajax = function(url,da,fun,type,efun, tsar){
	...
	var atyp = type.split(','),dtyp='';type=atyp[0],async=true;
