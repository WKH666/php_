...
(function ($) {
	...
	function bootstable(element, options){
		...
		this._create	= function(){
			...
			if(!can.hideHeaders){
				if(can.isshownumber)s+='<thead><tr><th width="50">序号</th>';
		...
		this.createrows=function(j){
			...
			if(s4)s5='rowspan="2"';
			if(!s3 && ((can.statuschange && ov.status==0) || (ov.ishui==1))){
				s3='style="color:#aaaaaa"';
			}else{
				s3='style="color: #5a5a5a;"';
			}
			s='<tr oi="'+j+'" dataid="'+ov.id+'" '+s3+'>';
			if(can.isshownumber)s+='<td '+s5+' align="center" width="40">'+(j+1+can.pageSize*(this.page-1))+'</td>';
			if(can.checked){
				dis = '';
		...
		this._loaddata = function(p, donbo){
			...
			s='<div id="modeshow_'+rand+'" style="filter:Alpha(opacity=20);opacity:0.2;display:block;height:100%;width:100%;position:absolute;z-index:3;top:1px;background:#000000;color:white;text-align: center; padding-top:15%;" align="center"><img src="images/mloading.gif"  align="absmiddle" />&nbsp;加载中...</div>';
			of=obj.offset();
			...
			if(o1.height()==0||o1.width()==0){
				h='100%';w='100%';
			}else{
				var h= o1.height()-2+'px',w= o1.width()-2+'px';
			}
			s='<div id="modeshow_'+rand+'" style="filter:Alpha(opacity=20);opacity:0.2;height:'+h+';width:'+w+';overflow:hidden;z-index:3;position:absolute;left:1px;line-height:'+h+'px;top:1px;background:#000000;color:white" align="center"><img src="images/mloading.gif"  align="absmiddle">&nbsp;加载中...</div>';
			//s='<div id="modeshow_'+rand+'" style="filter:Alpha(opacity=20);opacity:0.2;display:block;height:100%;width:100%;position:absolute;z-index:3;top:1px;background:#000000;color:white;text-align: center; padding-top:15%;" align="center"><img src="images/mloading.gif"  align="absmiddle" />&nbsp;加载中...</div>';
		...
		this.exceldown = function(bt){
			...
			$.ajax({
				...
				success:function(a1){
					js.msg('success', '处理成功，共有记录'+a1.totalCount+'条/导出'+a1.downCount+'条，点我直接<a class="a" href="'+a1.url+'" target="_blank" style="color: #CA4242;">[下载]</a>', 60);
					me.bool=false;
				},
				error:function(e){
	...
	$.fn.bootstable	= function(options){
		var defaultVal = {
			columns:[],		//表头
			selectColor:'rgb(233, 244, 255)', //选中时行颜色
			pageSize:15,   	//默认分页数
			limit:0,		//没有分页时展示条数
			bodyStyle:'',
			height:0,		//高度
