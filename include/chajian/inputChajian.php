	...
	*/
	public function getfieldcont($fid, $objs=null, $leox='', $iszb=0, $deval=false)
	{
		...
		if($fid=='base_deptname'){
			$str = '<input class="inputs" style="border:none;background:none;width: 250px;" name="base_deptname" value="'.$this->urs['deptname'].'" readonly>';
		}
		if($fid=='file_content'){
			$str = '<input name="fileid" type="hidden" id="fileidview-inputEl"><div id="view_fileidview" style="width:100%;height:80px;border:1px #000000 solid; background:white;overflow:auto"></div><div id="fileupaddbtn" class="cbtn_  tdcont1" style="display: inline-block;margin-top:2%;"><a href="javascript:;" class="blue" onclick="c.upload()" >＋添加文件</a></div>';
		}
		if($fid=='删'){
			$str='<a href="javascript:;" onclick="c.delrow(this,'.$xu0.')" >删除</a>';
		}
		if($fid=='新增'){
		...
		$str 	= '<input class="inputs" type="text" value="'.$val.'" '.$attr.' name="'.$fname.'"  placeholder="请填写">';
		...
		if($type=='textarea'){
			$str = '<textarea class="textarea"  '.$attr.' name="'.$fname.'"  placeholder="请填写">'.$val.'</textarea>';
		}
		if($type=='rockcombo' || $type=='select' || $type=='checkboxall'){
			$str ='<select '.$attr.' name="'.$fname.'" class="inputs">';
		...
		if($type=='datetime'||$type=='date'||$type=='time'||$type=='month'||$type=='year'){
			$str = '<input onclick="js.datechange(this,\''.$type.'\')" value="'.$val.'" '.$attr.' class="inputs datesss" inputtype="'.$type.'" readonly name="'.$fname.'" placeholder="请选择">';
		}
		if($type=='number'){
		...
		if($type=='changeusercheck'||$type=='changeuser'||$type=='changedept'||$type=='changedeptusercheck'){
			...
			$str 	= '<table width="98%" cellpadding="0" border="0"><tr><td width="100%"><input '.$attr.' class="inputs" style="width:96%" id="change'.$fname.'" value="'.$_vals0.'" readonly type="text" name="'.$fname.'"><input name="'.$data.'" value="'.$_vals1.'" id="change'.$fname.'_id" type="hidden"></td>';
			...
			//删除清空按钮
			$str   .= '<td nowrap><a href="javascript:;" id="btnchange_'.$fname.'" onclick="js.changeuser(\'change'.$fname.'\',\''.$type.'\')" class="webbtn" style="background-color:#AAAAAA">选择</a></td></tr></table>';
		}
		if($type=='selectdatafalse' || $type=='selectdatatrue'){
			...
			$str   .= '<td nowrap><a href="javascript:;" onclick="c.selectdata(\''.$data.'\','.substr($type,10).',\''.$fname.'\',\''.$fieldname.'\')" class="webbtn" style="background-color:#AAAAAA">选择</a></td></tr></table>';
		}
		if($type=='htmlediter'){
